...
function buildSystemPrompt(r: {
  ...
  return `You are the 24/7 AI receptionist for "${r.business_name}".
...
function mapAiError(e: unknown): Error {
  ...
  if (msg.includes("429")) return new Error("Too many requests, please slow down.");
  if (msg.includes("402")) return new Error("AI credits exhausted. Owner must top up.");
